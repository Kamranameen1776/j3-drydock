...
import { FilesService } from '../../../external-services/drydock/FilesService';
import {
...
export class ProjectDetailReport {
    ...
    private crtlName = 'description';
    private moduleCode = 'project';
    private functionCode = 'specification_details';
    private jibeLogo =
    ...
    public async getProjectDetailReport(projectUid: string, token: string, queryRunner: QueryRunner) {
        ...
        const [vesselType, vesselFields, specifications] = await Promise.all([
            this.vesselRepository.GetVesselType(Number(project.VesselType)),
            this.vesselRepository.GetVesselFields(Number(project.VesselId)),
            new SpecificationDetailsRepository().findSpecificationsForProjectReport(projectUid),
        ]);
        ...
        const appLocation = (await ConfigurationService.getConfiguration(eLocation.Location)) as eLocation;
        ...
        for (const spec of specifications) {
            attachmentPromises.push(
                ...
                ]).then(([files, media]) => (attachmentsBySpecification[spec.uid] = files.concat(media))),
            );

            if (spec.Description.includes('blob:')) {
                downloadPromises.push(this.upgradeDescription(token, spec, appLocation));
            }
        }
        ...
        await Promise.all([...attachmentPromises, ...downloadPromises]);
    ...
    private async upgradeDescription(token: string, spec: SpecificationForReport, appLocation: eLocation) {
        const { records: files } = await this.filesService.getAllFiles(
            token,
            this.moduleCode,
            this.functionCode,
            spec.uid,
        ...
        const document = htmlparser.parseDocument(spec.Description);

        await this.upgradeImagesToUrl(token, document, files, appLocation);

        spec.Description = render(document);
    }

    private async upgradeImagesToUrl(
        token: string,
        document: Document,
        ...
        return Promise.all(
            (selectAll('img[src^=blob:][id]', document) as unknown[] as Element[]).map(async (img) => {
                img.attribs.src =
                    (await this.filesService.getUrlForAttachment(token, files, img.attribs.id, appLocation)) ??
                    img.attribs.src;
            }),
        );
    }

    private async upgradeImagesToBase64(token: string, document: Document, files: JmsAttachmentsDetails[]) {
        return Promise.all(
            (selectAll('img[src^=blob:][id]', document) as unknown[] as Element[]).map(async (img) => {
                const file = files.find((f: any) => f.key3 === img.attribs.id);

                if (file) {
                    const [data, type] = await this.filesService.downloadAttachment(token, file.upload_uid);

                    img.attribs.src = `data:${type};${Buffer.from(data).toString('base64')}`;
                }
            }),
        );
    }
