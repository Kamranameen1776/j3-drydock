...
import { GetStandardJobsRequest } from '../../../application-layer/drydock/standard-jobs/dto/GetStandardJobsRequestDto';
...
async function getStandardJobs(req: express.Request, res: express.Response) {
    ...
    await middlewareHandler.ExecuteAsync(req, res, async (request: express.Request) => {
        const result = await new GetStandardJobsController().getStandardJobs(request as GetStandardJobsRequest);
...
export class GetStandardJobsController extends Controller {
    ...
    public async getStandardJobs(@Request() request: GetStandardJobsRequest): Promise<unknown> {
