...
async function AddPmsJob(req: Request, res: Response) {
    ...
    await middlewareHandler.ExecuteAsync(req, res, async (request: Request) => {
        const result = await new AddPmsJobController().AddPmsJob(request);
...
export class AddPmsJobController extends Controller {
    @Post()
    public async AddPmsJob(@Body() request: Request): Promise<void> {
        const query = new AddSpecificationPmsCommand();

        const result = await query.ExecuteAsync(request, UpdateSpecificationPmsDto);
