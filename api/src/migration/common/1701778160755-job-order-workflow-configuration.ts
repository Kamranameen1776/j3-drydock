...
export class jobOrderWorkflowConfiguration1701778160755 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        try {
            ...
            IF Exists(SELECT 1 FROM JMS_DTL_Workflow_config_details WHERE config_id in ( select Id from JMS_DTL_Workflow_config where active_status = 1 and JOB_Type = 'job_order'))
            BEGIN
               DECLARE @id int
               DECLARE workflow_config_Details CURSOR FOR
               select ID from JMS_DTL_Workflow_config_Details where config_id in (select id from JMS_DTL_Workflow_config where job_type='job_order' and active_status=1)
            ...
            BEGIN
                ...
                MERGE INTO TEC_LIB_Worklist_Type AS TARGET USING (
                        VALUES
                            (@WorkList_MAX_ID, N'job_order', N'JobOrder', 1, GETDATE(), 1, 0, NULL, 0, 0, null)
                    )
            ...
            END
            END
            ----------------------------------------------Insert 'Job Order' details in jms_dtl_workflow_config table--------------------------------------
                    BEGIN
                    DECLARE @MaxID int = 0,
                    @PkCondition varchar(100),
    
                    @JobType varchar(50) = 'job_order', --workflow set for which job type
                    ...
                    BEGIN
                        ...
                        VALUES
                        (@MaxID, @JobType, @TotalDays, @VesselAssign, @SubTask, @IncidentCategory, @MSCATCategory, @job_type_column_config)
                        )
                        AS SOURCE(ID, JOB_Type, Total_Days_Finalizing_Task, Vessel_Assigned, Sub_Task_Completed, is_incident_category, is_mscat_category, job_type_column_config)
                        ...
                        TARGET.Date_Of_Modification = getDate()
                        WHEN NOT MATCHED BY TARGET THEN
                        INSERT (ID,JOB_Type,Total_Days_Finalizing_Task,Vessel_Assigned,Sub_Task_Completed,is_incident_category,is_mscat_category,job_type_column_config, created_by,date_of_creation,active_status)
                        VALUES (
                        SOURCE.ID,
                        SOURCE.JOB_Type,
                ...
                -----------------------------------------Entry of 'Job Order' in jms_dtl_workflow_config_details table--------------------
                ...
                BEGIN
                    ...
                    IF Not Exists(SELECT 1 FROM JMS_DTL_Workflow_config_details WHERE config_id in ( select Id from JMS_DTL_Workflow_config where active_status = 1 and JOB_Type = 'job_order'))
                    BEGIN
                        ...
                        VALUES
                            (1, 'RAISE', 'Open', 'Create New', 0, 0, null,'["proj_drd_job_order_raise_office"]'),
                            (2, 'IN PROGRESS', 'Update Spec', 'In-Progress', 1, 0, 1,'["proj_drd_job_order_in_progress_office"]'),
                            (3, 'COMPLETE', 'Complete Spec', 'Completed', 1, 1, 1,'["proj_drd_job_order_complete_office"]'),
                            (4, 'CANCEL', 'Cancel', 'Cancelled', 1, 1, 1,'["proj_drd_job_order_cancel_office"]')
                        ...
                        DECLARE wl_type_cursor CURSOR LOCAL FOR
                            select Id, JOB_Type from JMS_DTL_Workflow_config where active_status = 1 and JOB_Type = 'job_order'
                        OPEN wl_type_cursor
                                        ...
                                        BEGIN
                                            BEGIN TRY
                                                ...
                                                IF(@Config_Details_Id > 0)
                                                    ...
                                                    BEGIN
                                                    Set @Active_Status = 1
                                                    SET @right_code='["proj_drd_job_order_raise_office"]'
                                                    END
                                                    ELSE IF (@Is_Office =0)
                                                    BEGIN
                                                    Set @Active_Status = 0
                                                    SET @right_code='["proj_drd_job_order_raise_vessel"]'
                                                    END
            ...
            (@config_id,'Cancel','{"validation":{"is_mandatory":true}}', null)  
            )
