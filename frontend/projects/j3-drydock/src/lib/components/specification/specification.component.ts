...
import { Specification, SpecificationService, SpecificationStatus, SpecificationType } from './specification.service';
...
export class SpecificationComponent implements OnInit, OnDestroy {
  ...
  constructor(private specsService: SpecificationService, private jmsTechService: JmsTechApiService) {}
  ...
  private callData(type: SpecificationType) {
    this.specsService.getGridData('', type).pipe(takeUntil(this.$destroySubject)).subscribe((data) => {
      this.gridData = data
